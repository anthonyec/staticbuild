...
async function cleanOutputDirectory(outputDirectory, pages, assets) {
    const expectedOutputPaths = getExpectedOutputPaths(outputDirectory, pages, assets).map((outputPath) => path.join(outputDirectory, outputPath));
    await (0, fs_1.scanDirectory)(outputDirectory, [], async (file) => {
        // TODO: Rename this long variable.
        const expectedOutputPathsThatStartWithFilePath = expectedOutputPaths.find((expectedOutputPath) => {
